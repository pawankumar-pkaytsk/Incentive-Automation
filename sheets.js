...
(function () {
  ...
  // Sellers ops has confirmed ARE 3-week HITs but which the 3weekgolive sheet omits.
  //
  // That sheet is generated by Metabase card 10454, whose ACTIVE filter is
  //     hit_year_week - go_live_week <= 3
  // while a commented-out line directly above it uses (go_live_week + 1) -- one week looser.
  // 277 sellers sit in exactly that gap, each a x1.5 HIT under one reading and x1 under the
  // other. Changing the card would move all 277 at once, so confirmed individual cases are
  // listed here instead. Each entry is a manual assertion: revisit if the card's rule is settled.
  const THREE_WEEK_OVERRIDES = {
    // Icchaa Fashion / ASHWINI AMIT PARASNIS -- HIT week 202631, go-live week 202627, gap 4.
    // Qualifies under the commented-out rule, not the active one. Owner-confirmed 2026-08-26.
    '6a3147af962d97d03ffe2bbb': true,
  };
  ...
  function computeAll(RAW) {
    ...
    Object.keys(THREE_WEEK_OVERRIDES).forEach((id) => { if (THREE_WEEK_OVERRIDES[id]) threeWeek[id] = true; });
